    ...
    <>
      <nav className='navbar flex flex-col justify-around items-center w-full h-auto border-b-2 border-[var(--fg)]'>
        ...
        <div className='flex w-full flex-row items-center px-4 py-2 gap-2'>
          ...
          <span className='text-[var(--text)]'>Bienvenido <span className='font-bold uppercase text-[var(--text)]'>{User}</span></span>

        </div>
        <div className='routes z-11 flex justify-center items-center flex-col gap-4 text-lg fixed right-0 top-0 h-full w-1/2 bg-[var(--fg)] px-4 py-2 hidden'>
          <Link className='cursor-pointer text-[var(--text)] hover:text-black' to="/">Publicaciones</Link>
          <Link className='cursor-pointer text-[var(--text)] hover:text-black' to="/inventory">Inventario</Link>
